...
const Button = ({
  ...
  // Nếu có custom className, không apply variant styles
  const finalClassName = className 
    ? `${baseStyles} ${sizes[size]} ${widthClass} ${disabled || loading ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'} ${className}`
    : `${baseStyles} ${variants[variant]} ${sizes[size]} ${widthClass} ${disabled || loading ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'}`;
